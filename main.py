...
class Processor():
    def __init__(self, arg):
        ...
        self.mlp=self.mlp.cuda(self.output_device)
...
# 3 优化器配置，如果使用需要更新参数的loss，需要把loss置入损失函数
    ...
    def sq_loss(self,output,label):
        # print(output.shape)
        loss = torch.pow((output - label), 2)+torch.abs(output-label)
    ...
    def get_acc(self,output, label):
        predict_label = torch.where(output >=0.29, 1, 0)
    ...
    def train(self, epoch, save_model=False,grad_norm=False):
        ...
        for batch_idx, (img, feature , label, index) in enumerate(process):
            self.global_step += 1
            with torch.no_grad():
                img = img.float().cuda(self.output_device)
                feature = feature.float().cuda(self.output_device)
                label = label.float().cuda(self.output_device)

            timer['dataloader'] += self.split_time()
            img = self.model(img,feature)
            output=self.mlp(img,feature)
    ...
    def eval(self, epoch, save_score=False, loader_name=['test']):
        ...
        for ln in loader_name:
            ...
            if save_score:
                ...
                label_pth=np.concatenate(label_list,axis=0)
                print(score_pth.shape)
                with open(self.arg.work_dir+'/sc.pkl','wb') as f:
                    pickle.dump(score_pth,f)
                with open(self.arg.work_dir+'/label.pkl','wb') as f:
                    pickle.dump(label_pth,f)
